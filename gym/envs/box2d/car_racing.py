...
class FrictionDetector(contactListener):
    ...
    def _contact(self, contact, begin):
        ...
        if begin:
            ...
            # print tile.road_friction, "DEL", len(obj.tiles) -- should delete
            # to zero when on grass (this works)
...
class CarRacing(gym.Env):
    ...
    def __init__(self):
        ...
        self.action_space = spaces.Box(
            np.array([-1, 0, 0]), np.array([+1, +1, +1]))  # steer, gas, brake
    ...
    # Custom function for IR state
    def _get_ir_state(self):
        ir_state = [0] * 6
        for ce in self.car.hull.contacts:
            contact = ce.contact
            fix1 = contact.fixtureA
            fix2 = contact.fixtureB
            if fix1 and "road_friction" in fix1.body.userData.__dict__:
                if fix2.shape.__class__.__name__ == 'b2CircleShape':
                    if contact.touching or ir_state[fix2.userData]:
                        ir_state[fix2.userData] = 1
                    else:
                        ir_state[fix2.userData] = 0
    ...
    def _render_indicators(self, W, H):
        ...
        true_speed = np.sqrt(np.square(self.car.hull.linearVelocity[
                             0]) + np.square(self.car.hull.linearVelocity[1]))
...
if __name__ == "__main__":
    ...
    while True:
        ...
        while True:
            ...
            steps += 1
            # Faster, but you can as well call env.render() every time to play
            # full window.
            if not record_video:
                env.render()
            if done or restart:
